...
const EngagingLoader: React.FC<EngagingLoaderProps> = ({ status, message, onRetry }) => {
    if (status !== 'loading' && status !== 'error') {
        return null;
    }

    return (
        <div className="absolute inset-0 z-30 bg-black/80 backdrop-blur-sm flex flex-col items-center justify-center gap-6 text-white p-4 animate-fade-in">
            {status === 'loading' && (
                <>
