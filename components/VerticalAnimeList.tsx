...
const VerticalAnimeListItem: React.FC<{ anime: Anime; onSelect: (anime: Anime) => void; rank: number }> = ({ anime, onSelect, rank }) => {
  ...
  return (
    ...
    >
        {rank > 0 && (
            <div className="absolute -left-2 top-1/2 -translate-y-1/2 z-0 pointer-events-none" aria-hidden="true">
                <span className="text-6xl font-black text-transparent" style={{ WebkitTextStroke: '2px rgba(34, 211, 238, 0.7)' }}>
                    {rank}
                </span>
            </div>
        )}
      <div className="flex items-center gap-4 relative z-10">
        <div className={`w-16 h-24 flex-shrink-0 ${rank > 0 ? 'ml-6' : ''}`}>
            <img 
